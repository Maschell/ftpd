...
INITIALIZE_PLUGIN() {
	...
	if (storageRes != WUPS_STORAGE_ERROR_SUCCESS) {
		OSReport("Failed to open storage %s (%d)\n", WUPS_GetStorageStatusStr(storageRes), storageRes);
	} else {
		if ((storageRes = WUPS_GetBool(nullptr, FTPIIU_ENABLED_STRING, &sFTPServerEnabled)) == WUPS_STORAGE_ERROR_NOT_FOUND) {
			// Add the value to the storage if it's missing.
			if (WUPS_StoreBool(nullptr, FTPIIU_ENABLED_STRING, sFTPServerEnabled) != WUPS_STORAGE_ERROR_SUCCESS) {
				OSReport("Failed to store bool\n");
			}
		} else if (storageRes != WUPS_STORAGE_ERROR_SUCCESS) {
			OSReport("Failed to get bool %s (%d)\n", WUPS_GetStorageStatusStr(storageRes), storageRes);
		}
		if ((storageRes = WUPS_GetBool(nullptr, SYSTEM_FILES_ALLOWED_STRING, &sSystemFilesAllowed)) == WUPS_STORAGE_ERROR_NOT_FOUND) {
			// Add the value to the storage if it's missing.
			if (WUPS_StoreBool(nullptr, SYSTEM_FILES_ALLOWED_STRING, sSystemFilesAllowed) != WUPS_STORAGE_ERROR_SUCCESS) {
				OSReport("Failed to store bool\n");
			}
		} else if (storageRes != WUPS_STORAGE_ERROR_SUCCESS) {
			OSReport("Failed to get bool %s (%d)\n", WUPS_GetStorageStatusStr(storageRes), storageRes);
...
void gFTPServerRunningChanged (ConfigItemBoolean *item, bool newValue)
{
	...
	auto res = WUPS_StoreInt (nullptr, FTPIIU_ENABLED_STRING, sFTPServerEnabled);
	if (res != WUPS_STORAGE_ERROR_SUCCESS)
	{
		OSReport (
		    "Failed to store gFTPServerEnabled: %s (%d)\n", WUPS_GetStorageStatusStr (res), res);
	}
}
...
void gSystemFilesAllowedChanged (ConfigItemBoolean *item, bool newValue)
{
	...
	// If the value has changed, we store it in the storage.
	auto res = WUPS_StoreInt (nullptr, SYSTEM_FILES_ALLOWED_STRING, sSystemFilesAllowed);
	if (res != WUPS_STORAGE_ERROR_SUCCESS)
	{
		OSReport (
...
WUPS_GET_CONFIG ()
{
	...
	{
		OSReport ("Failed to open storage\n");
		return 0;
	}
